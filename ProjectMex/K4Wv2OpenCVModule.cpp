...
CK4Wv2OpenCVModule::CK4Wv2OpenCVModule()
{
	...
	pInfraRAWBuffer = new UINT16[INFRARED_FRAME_WIDTH * INFRARED_FRAME_HEIGHT];
	...
	memset(pDepthRAWBuffer, 0, DEPTH_FRAME_WIDTH * DEPTH_FRAME_HEIGHT * sizeof(UINT16));
	memset(pInfraRAWBuffer, 0, INFRARED_FRAME_WIDTH * INFRARED_FRAME_HEIGHT * sizeof(UINT16));
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hrColor) && SUCCEEDED(hrDepth))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if ((depthBufferSize == DEPTH_FRAME_WIDTH * DEPTH_FRAME_HEIGHT) && (dynamicData != NULL))
				{
					// Copy depth data
					memcpy(dynamicData, pTmpDepthBuffer, depthBufferSize * sizeof(UINT16));
				}
